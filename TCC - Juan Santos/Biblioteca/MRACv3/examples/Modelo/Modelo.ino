...
// Esse exemplo é para mostrar o uso das funções Modelo() e mostraModelo()
// onde é possível modificar o ganho e as constantes da Função de Transferência modelo utilizada.
// Lembrando que a Função de Transferência do modelo se dá por:
// M(s) = K * a/(s+a)
// ou seja, essa função modifica o valor de K, o valor da constante que acompanha "s" e o valor de "a", respectivamente.


// AVISOS:
// Tanto Modelo() como mostraModelo() funcionam no void setup() e void loop();
// Não é recomendando o uso de mostraModelo() no void loop() caso esteja sendo utilizado o Serial Plotter;
...
void setup() {
  ...
  // Pode ser removido se não for usar a função mostraModelo()
  Serial.begin(9600);

  // Voce pode modificar os valores do modelo antes do loop iniciar
  modulo1.Modelo(2, 2, 30);
...
void loop() {
  ...
  if (5000 <= (millis() - comeco)) { // dps de 5 segundos do inicio
    modulo1.Modelo(1, 1, 20); // Modifica novamente os dados, agora durante o seu funcionamento
